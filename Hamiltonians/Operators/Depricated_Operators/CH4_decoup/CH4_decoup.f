...
c----------------------------------------------------------------------
c
c Fri, 23 Jan 2009 10:41:23 (CET)  GS, RW
c 5d frozen: theta_rho, phi_rho, phi, chi frozen
c
c######################################################################

      subroutine h initch4decoup(koeff,diag,dim)
...
c changes constants due to coupling terms
c [7,7]
      do i=13,22
         c(i)=1.d0
      enddo
...
c Bestimmung der diagonalen Komponenten des Hamilton-Operators
      koeff(kzahl+1)=0
      call diagondecoup(diag,dim,kzahl)
...
c--------------------------------------------------------------
      ...
      subroutine diagondecoup(diag,dim,k zahl)
      ...
      do k=1,kzahl
         do i=1,dim
            ort=998.0d0
            call hch4decoup(i,k,hpsi,psi,1,matrix,trafo,ort)
...
c#####################################################################
      ...
      subroutine hch4decoup (modesmall,teil,h psi,psi,dim,matrix,
     . trafo,ort)

      implicit      none

      integer       mode, modesmall, teil, dim, i
      ...
      mode = -1
      if (modesmall.eq.1) mode = 1
      if (modesmall.eq.2) mode = 4
      if (modesmall.eq.3) mode = 7
      if (modesmall.eq.4) mode = 8
      if (modesmall.eq.5) mode = 9
...
104   goto (
     & 1, 1, 1, 1,10,  30, 8, 1, 8, 1,   24,11,88,88,88, 88,88,88,88,88,
     &88,88,                                                            
...
c H = 2/(1+cos(x)**2)
 ...
 88   do i=1,dim
         h psi(i)=2.d0*psi(i)/(1.d0 + cos(ort(i))**2)
      end do
      return
...
c -------------------------------------------------------------------
      ...
      subroutine IntToCartch4decoup(q,x)
