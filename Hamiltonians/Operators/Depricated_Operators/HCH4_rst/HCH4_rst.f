...
c
c----------------------------------------------------------------------
c For H+CH4 (coordinates by Evenhuis/Manthe (CH3) and Schiffel/Manthe (RST))
c
c######################################################################

      subroutine h inithch4rst(koeff,diag,dim)
      use iso_c_binding
...
c Bestimmung der diagonalen Komponenten des Hamilton-Operators
      koeff(kzahl+1)=0
      call diagonhch4rst(diag,dim,kzahl)
...
c--------------------------------------------------------------
      ...
      subroutine diagonhch4rst(diag,dim,k zahl)
      use iso_c_binding

      integer     dim, k zahl, i, k
c      logical*4   diag(dim,k zahl)
...
c            diag(i,k)=.false.
      ...
      do k=1,kzahl
         do i=1,dim
            ort=998.0d0
            call hhch4rst(i,k,hpsi,psi,1,matrix,trafo,ort)
...
c#####################################################################
      ...
      subroutine hhch4rst (mode,teil,h psi,psi,dim,matrix,trafo,ort)
...
25    do i=1,dim
         work(i)=psi(i)/sqrt(xphihch4(ort(i)))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         work(i)=hpsi(i)*xphihch4(ort(i))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)/sqrt(xphihch4(ort(i)))
      end do
      return 
...
26    do i=1,dim
         work(i)=psi(i)/sqrt(xchihch4(ort(i)))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         work(i)=hpsi(i)*xchihch4(ort(i))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)/sqrt(xchihch4(ort(i)))
      end do
      return 
...
27    do i=1,dim
      ...
      do i=1,dim
         work(i)=sqrt(xphihch4(ort(i)))*psi(i)
      end do
      call ddx (mode,work2,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)-work2(i)/sqrt((xphihch4(ort(i))))
      enddo
      return
...
c -------------------------------------------------------------------
      ...
      xphihch4=(1.0d0/( 1.0d0 +                   (phi-pi3)**2
     .                    - sqrt(3.0d0)/9.0d0*(phi-pi3)**3
     .                    +       3.0d0/4.0d0*(phi-pi3)**4  ))

      end 
c -------------------------------------------------------------------
      real*8 function xchihch4(chi)
      implicit none
      ...
      xchihch4=( 1.0d0/( 1.0d0 + ((chi-pi)**2)/3.0d0
     .                     + ((chi-pi)**4)/12.0d0 ))

      end

c -------------------------------------------------------------------
      subroutine koordreadhch4rst
      use iso_c_binding
      implicit none

      integer    na,na3,dimch3
      parameter (na=6,na3=3*na,dimch3=12)
...
c ------------------------------------------------------------------------
      ...
      subroutine IntToCarthch4rst(q,x) 
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter  (na=6,na3=3*na,dimq=na3-6,dimch3=12)
...
c reduced mass between CH4 and 5th H
      ...
      rmch4=mch4*m(6)/(mch4+m(6))
      srmch41=1.0d0/sqrt(rmch4)
...
c Fifth step: add 5th hydrogen atom

c  unmassweight R
      dist2=q(10)*srmch41
      nenn2=1.0d0/(1.0d0+q(11)**2+q(12)**2)

c    Center of mass for first 5 atoms
      xcm(1)=0.0d0
      xcm(2)=0.0d0
      xcm(3)=0.0d0
      do i=1,5
      ...
      x(16)=xcm(1)+2*dist2*q(11)*nenn2
      x(17)=xcm(2)+2*dist2*q(12)*nenn2
      x(18)=xcm(3)+dist2*(1.0d0-q(11)**2-q(12)**2)*nenn2
