...
c######################################################################
      ...
      subroutine h initabc3rst(koeff,diag,dim)
      use iso_c_binding
      implicit none
...
c Bestimmung der diagonalen Komponenten des Hamilton-Operators
      koeff(kzahl+1)=0
      call diagonabc3rst(diag,dim,kzahl)
...
c--------------------------------------------------------------
      ...
      subroutine diagonabc3rst(diag,dim,k zahl)
      use iso_c_binding
...
c            diag(i,k)=.false.
      ...
      do k=1,kzahl
         do i=1,dim
            ort=998.0d0
            call h abc3rst(i,k,hpsi,psi,1,matrix,trafo,ort)
...
c#####################################################################
      ...
      subroutine h abc3rst (mode,teil,h psi,psi,dim,matrix,trafo,ort)
...
25    do i=1,dim
         work(i)=psi(i)/sqrt(xphiabc3(ort(i)))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         work(i)=hpsi(i)*xphiabc3(ort(i))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)/sqrt(xphiabc3(ort(i)))
      end do
      return
...
26    do i=1,dim
         work(i)=psi(i)/sqrt(xchiabc3(ort(i)))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         work(i)=hpsi(i)*xchiabc3(ort(i))
      end do
      call ddx (mode,h psi,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)/sqrt(xchiabc3(ort(i)))
      end do
      return
...
27    do i=1,dim
      ...
      do i=1,dim
         work(i)=sqrt(xphiabc3(ort(i)))*psi(i)
      end do
      call ddx (mode,work2,work,dim,matrix,trafo)
      do i=1,dim
         hpsi(i)=hpsi(i)-work2(i)/sqrt((xphiabc3(ort(i))))
      enddo
      return
...
c -------------------------------------------------------------------
      ...
      xphiabc3=(1.0d0/( 1.0d0 +                   (phi-pi3)**2
     .                    - sqrt(3.0d0)/9.0d0*(phi-pi3)**3
     .                    +       3.0d0/4.0d0*(phi-pi3)**4  ))

      end
c -------------------------------------------------------------------
      real*8 function xchiabc3(chi)
      implicit none
      ...
      xchiabc3=( 1.0d0/( 1.0d0 + ((chi-pi)**2)/3.0d0
     .                     + ((chi-pi)**4)/12.0d0 ))

      end
c -------------------------------------------------------------------

      subroutine IntToCartabc3rst(q,x)
...
c ------------------------------------------------------------------------
      subroutine koordreadabc3rst(mass)
      implicit none

      integer    na,na3,dimch3
      parameter (na=5,na3=3*na,dimch3=12)

      real*8     mch3,hmass, mass(5)
      parameter (hmass=1836.109d0)
