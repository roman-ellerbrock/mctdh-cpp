...
double NOClPotential::Evaluate(const Vectord & Xv, size_t part) {
    if (dissociation) {
...
double NOClPotential::EvaluateGS(const Vectord &Xv, size_t part) {
...
double NOClPotential::EvaluateS1(const Vectord & Xv, size_t part)
{
...
/*	if (rd < (1. - exp(-1.5*(3.5 - 4.315))))
	...
	*/
	...
	double      v =
		...
		1.03112*pow(a, 4) + 0.28978*pow(a, 5) + 0.878604*pow(a, 6))*pow(rd, 4))*pow(rv, 3))*(1. - rd);
